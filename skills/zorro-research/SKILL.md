...
> 版本：v1.6 | 2026-04-29
> 优化：入库强制规则，status=raw 不得入库，必须走 tweet-to-wiki skill
...
2. 用 Camofox（Python脚本）抓取推文全文
3. Luffy 筛选 P1-P3 内容（5-10条）
4. 生成结构化周报（150-200字/条，含"为什么有用"字段）
5. 入库：`~/wiki/twitter/weekly-ai-insights/`

**Camofox X抓取（必须严格按此顺序）：**
1. **Cookie 同步**：`bash ~/.hermes/scripts/sync-x-cookies.sh`（每次抓取前必须运行）
2. **抓取单条**：`python3 ~/.hermes/scripts/x_scrape.py <推文URL>`
3. **批量下载**：`python3 ~/.hermes/scripts/x_batch_download.py <推文URL1> <推文URL2> ...`
...
**Camofox状态**：`curl -s http://localhost:9377/health`
...
**来源A — X/Twitter 搜索（web_search）：**
```
site:x.com AI agent OR "multi-agent" OR "second brain" OR "knowledge base" OR "Claude Code" 2026
```

**来源B — Camofox 抓取（已知账号）：**
⚠️ `x_batch_download.py` **不能**抓取主页 URL（如 `https://x.com/dotey` 会返回 "Invalid URL"），**只能**抓取单条推文 URL。

推荐做法：
1. 先检查 `~/wiki/twitter/weekly-ai-insights/raw/` 目录——已有预下载的原始推文，可直接分析
2. 如需新抓取，需要先从其他来源获取单条推文 URL，再用 `x_scrape.py` 或 `x_batch_download.py`
...
- 推文原文：`~/wiki/twitter/{author}/{tweet_id}.md`
- frontmatter 必须包含：`source`, `url`, `date`, `author`, `type: tweet`, `tags`, `status: curated`
...
**入库流程必须走 `tweet-to-wiki` skill**：
- 抓取 → x_scrape.py（输出 status: raw）
- 入库前 → 将 status 改为 curated
- 不允许：直接保存 raw 输出到 wiki 目录
