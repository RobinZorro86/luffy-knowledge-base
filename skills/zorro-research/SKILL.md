...
3. **抓取单条**：
   ```bash
   bun ~/.hermes/skills/baoyu-danger-x-to-markdown/scripts/main.ts <推文URL>
...
3. 读取所有候选文件，验证 frontmatter（status=curated，frontmatter 完整）
4. 评分并推送入库通知
5. 扫描完成后将日志写入 tweet-discovery-log.md
...
- 推文原文：`~/wiki/twitter/{author}/{tweet_id}.md`（**必须用 tweet_id，不能用 slug**）
...
date: "2026-05-01"          # 推文发布日期（从 url 或内容中提取）
type: tweet
tags: ["主题标签1", "主题标签2"]
...
**入库流程必须走 baoyu-x-to-markdown + 标准化 frontmatter**：
